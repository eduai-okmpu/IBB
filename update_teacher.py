...
new_db = """const physicsDb = {
  questions: {
    ...
    "Термодинамика": [
        ...
        { q: "Менделеев-Клапейрон теңдеуі?", options: ["PV = nRT", "PV = nRT", "P/T = R", "V = nRT"], answer: 0 },
    ...
    "Электр және магнетизм": [
        ...
        { q: "Ойыс айна дегеніміз?", options: ["Шағылыстыратын беті ішке қарай иілген айна", "Тегіс айна", "Дөңес айна", "Шыны"], answer: 0 },
...
# Replace physicsDb
content = re.sub(r'const physicsDb = \{.*?\}\s*;\s*', new_db + ";\n\n", content, flags=re.DOTALL)

# Fix Timer Logic (prevent negative values and off-by-one display)
timer_pattern = r'quizSession\.timer = setInterval\(\(\) => \{.*?\}, 1000\);'
new_timer = """quizSession.timer = setInterval(() => {
