...
const filePath = '/Users/yesbolgansattar/Desktop/ИББ сайт/js/teacher.js';

const physicsDbContent = `const physicsDb = {
  questions: {
    "Механика": [
        ...
        { q: "Механикалық тербеліс жиілігіның өлшем бірлігі?", options: ["Герц (Гц)", "Секунд (с)", "Метр (м)", "Ньютон (Н)"], answer: 0 },
    ...
    "Оптика": [
        ...
        { q: "Күн сәулесіның жеті түске бөлінуін алғаш зерттеген кім?", options: ["Исаак Ньютон", "Альберт Эйнштейн", "Галилео Галилей", "Макс Планк"], answer: 0 },
...
const resultsLogicReplacement = `function renderQuizResults() {
...
const timerLogicReplacement = `quizSession.timer = setInterval(() => {
...
try {
    ...
    // 1. Replace physicsDb
    content = content.replace(/const physicsDb = \{[\s\S]*?\}\s*;\s*/, physicsDbContent + ";\\n\\n");

    // 2. Fix Timer Logic
    const timerPattern = /quizSession\\.timer = setInterval\(\(\) => \{[\s\S]*?\}, 1000\);/;
    content = content.replace(timerPattern, timerLogicReplacement);

    // 3. Fix renderQuizResults audio and flow
    content = content.replace("function renderQuizResults() {", resultsLogicReplacement);

    fs.writeFileSync(filePath, content, 'utf8');
    console.log("Successfully updated teacher.js using Node.js");
} catch (err) {
    console.error("Error updating teacher.js:", err);
    process.exit(1);
}
