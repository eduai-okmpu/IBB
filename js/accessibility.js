...
function initVoices() {
  ...
  // 1-қадам: Нақты қазақша дауысты іздеу (Google немесе Microsoft)
  let bestVoice = voices.find(v => v.lang === 'kk-KZ' || v.lang === 'kk_KZ');

  // 2-қадам: Егер табылмаса, атында 'Kazakh' сөзі бар кез келгенін іздеу
  if (!bestVoice) {
    bestVoice = voices.find(v => v.name.includes('Kazakh'));
  }

  a11yState.kazakhVoice = bestVoice;
}
